...
struct DestiniBusinessLogic {
    var storyCount = 0
    
    let destini: [Story] = [Story(title: "Your car has blown a tire on a winding road in the middle of nowhere with no cell phone reception. You decide to hitchhike. A rusty                                       pickup truck rumbles to a stop next to you. A man with a wide brimmed hat with soulless eyes opens the passenger door for you and                                        asks: 'Need a ride,boy?'.",
    ...
    mutating func storyPath(story userChoice: String) {
        if userChoice == destini[storyCount].choice1{
             storyCount = destini[storyCount].choice1Destination
        } else if userChoice == destini[storyCount].choice2{
             storyCount = destini[storyCount].choice2Destination
        }
    }
    
    func getTitle() -> String {
        return destini[storyCount].title
    }
    func getChoice1() -> String {
        return destini[storyCount].choice1
    }
    func getChoice2() -> String {
        return destini[storyCount].choice2
    }
    func getChoice1Destination() -> Int {
        return destini[storyCount].choice1Destination
    }
    func getChoice2Destination() -> Int {
        return destini[storyCount].choice2Destination
    }
}
